...
public class Id2Id {
	...
	public static JSONArray getPath4Id2Id(final Entity src, final Entity dst) throws Exception {
		...
		Future<List<Entity>> getSrcPaperRefsFuture = executor.submit(new IIGetSrcPaperRefsThread(src));
		Future<List<Entity>> getDstPaperReferersFuture = executor.submit(new IIGetDstPaperReferersThread(dst));
		executor.shutdown();
		...
		while (!(getSrcPaperRefsFuture.isDone() || getDstPaperReferersFuture.isDone()))
			;
...
class IIGetSrcPaperRefsThread implements Callable<List<Entity>> {
	private Entity src;

	public IIGetSrcPaperRefsThread(Entity src) {
		this.src = src;
	}
	...
	public List<Entity> call() throws Exception {
		...
		if (src.references != null) {
			try {
				entityList = BOPEntityManager.getSrcPaperRefs4II(src.references);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
...
class IIGetDstPaperReferersThread implements Callable<List<Entity>> {
	private Entity dst;

	public IIGetDstPaperReferersThread(Entity dst) {
		this.dst = dst;
	}
	...
	public List<Entity> call() throws Exception {
		List<Entity> entityList = new ArrayList<>();
		try {
			entityList = BOPEntityManager.getDstPaperReferers4II(dst.entityId);
		} catch (Exception e) {
		}
		return entityList;
	}
