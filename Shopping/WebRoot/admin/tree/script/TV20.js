...
//========================================
tv_listeners = new Array() ;//监听器 事件模型原理observer 一系列监听器数组
function listener( type , handler )   {//JavaScript 面向对象 定义一个function的同时 定义了一个对象 和构造方法
	this.type = type ;
...
function addListener( type , handler )  {
    new listener( type , handler ) ;  //面向对象
 }
//=== END =====
...
//=================================
//Custom a stack 为了用递归 但是javascript要自己写栈
//Class         : stack
//metheds    :  get()
//                    put( obj )
//=================================
...
//stack这个对象的原型里有一个方法叫get 当我们用这个方法时实际是stack_get 相当于把这两个方法绑定在一起
...
function nodeitem( parentkey , key , lable , img )  {
	...
	this.img = img ;
	this.tag = null ;
    this.status = "100" ;//为什么要用string 分别的三个位置不可以吗
