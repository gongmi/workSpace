...
<script type="text/javascript">
	var req;
	function validate() {
		var idField = document.getElementById("userid");//idField是创建一个用户时我敲的用户名
		var url = "Validate.jsp?id=" + escape(idField.value);//这个函数是转换参数
		if(window.XMLHttpRequest) {//ajax 书 p34
			req = new XMLHttpRequest();
		} else if (window.ActiveXObject) {
			req = new ActiveXObject("Microsoft.XMLHTTP");
		}
		req.open("GET", url, true);
		req.onreadystatechange = callback;// 类似于onclick
		req.send(null);
	}
...
<td class="altbg2">
<input id="userid" name="username" size="25" maxlength="25" type="text" onblur="validate()"> 
<span id="usermsg">哈哈哈哈哈</span> 
<!--上面是用户名右边的一个span 用来显示反馈的结果  -->
</tr>
